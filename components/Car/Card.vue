...
<template>
     <div class="relative shadow border w-full overflow-hidden mb-5 cursor-pointer h-[200px]">
        <img :src="favored ? heartFilled : heartOutline" class="absolute w-7 right-5 top-2 z-20" @click="emit('favor', car.id)" alt="Favorite">
        <div class="flex h-full" @click="navigateTo(`/car/${car.name}-${car.id}`)">
            <NuxtImg :src="car.url" :alt="car.name" class="w-[300px] h-full object-cover" />
